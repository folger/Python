    ...
    for i, line in enumerate(data):
        ...
        if m:
            ...
            if not comment:
                for j in range(10):
                    pre_line = data[i-j-1]
                    comment = p_comment_one_line.match(pre_line)
    ...
    for sysvar in sysvars:
        ...
        for fmt in date_formats:
            try:
                ...
                pass
        if date >= date_cmp:
            comment_detail = p_comment_detail.match(comment.group(0).lstrip())
            print('{}\t{}\t{}'.format(var, comment_detail.group(1),
                                      comment_detail.group(2)), file=fw)
